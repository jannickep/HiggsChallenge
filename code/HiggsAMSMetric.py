...
def create_solution_dictionary(solution):
    ...
    print "creating solution dictionary"
    solnDict = {}
...
def AMS_metric(solution, submission):
    ...
    if check_submission(submission, numEvents):
        print "passed submission check"
        with open(submission, 'rb') as f:
            print "opened submission"
            sub = csv.reader(f)
            sub.next() # header row
            for row in sub:
...
if __name__ == "__main__":
    ...
    path = ""
    solutionFile = "../data/fake-solution-file.csv"
